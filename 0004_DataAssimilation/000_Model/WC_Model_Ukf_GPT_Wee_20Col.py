...
# -*- coding: utf-8 -*-
"""
Wilson–Cowan Multi-column + UKF
観測: Exc 活動 E_i (i=1..Nc)
推定状態: [E_1..E_Nc, I_1..I_Nc, W_EE]
推定対象パラメータ: W_EE
"""
import numpy as np
...
Z_obs = E_true.T + rng.normal(0.0, OBS_NOISE_STD, size=(N_step, Nc))

# ---------- UKF セットアップ ----------
current_k = 0  # グローバルに現在時刻インデックスを管理

dim_x = 2 * Nc + 1   # [E1..ENc, I1..INc, W_EE]
dim_z = Nc           # [E1..ENc]
...
# 初期推定値
ukf.x = np.hstack([Z_obs[0], np.zeros(Nc), 10.0])
P_E0 = 0.1
P_I0 = 0.1
P_W0 = 1.0
