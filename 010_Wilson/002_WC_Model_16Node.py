...
T_total = 200.0            # 総シミュレーション時間 [ms]
Step    = int(T_total / dt) # ステップ数
time    = np.arange(Step) * dt

# ---------- ノード数設定 (Path Graph N=16) ----------
Nc = 16  # Path Graph のノード数
...
# ---------- Path Graph の隣接行列 ----------
# ノード i は i-1, i+1 と結合 (端点は一つのみ)
W_inter = np.diag(np.ones(Nc-1), 1) + np.diag(np.ones(Nc-1), -1)
C_inter = 0.7  # グローバル結合スケール
...
def wc_deriv(E, I, P_t, Q_t, wEE, wEI, wIE, wII, C, W):
    inter = C * (W @ E)
    dE = (-E + (1 - r_E * E) * S_E(wEE * E - wEI * I + P_t + inter)) / tau_E
    dI = (-I + (1 - r_I * I) * S_I(wIE * E - wII * I + Q_t)) / tau_I
    return dE, dI

for k in tqdm(range(Step-1), desc='RK4 Path Graph N=16'):
...
plt.title('Excitatory activities (Path Graph N=16)')
...
sns.heatmap(E_true, cmap='viridis', cbar_kws={'label': 'E activity'},
            yticklabels=[f'N{i+1}' for i in range(Nc)], xticklabels=Step//20)
plt.xlabel('Time step')
plt.ylabel('Node')
plt.title('Excitatory Heatmap (Path Graph N=16)')
plt.show()
