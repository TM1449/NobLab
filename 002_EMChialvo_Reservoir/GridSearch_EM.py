...
class Normal:
    ...
    def Load_AllLogs(self, save_type: str) -> list:
        ...
        if Paths is not None:
            for path in Paths:
                Dir_Logs_Branch = self.Dir_Logs.AddTemporaryChild(FileAndDir_EM.DirNode(path))
                Dir_Logs_Branch_Log = Dir_Logs_Branch.AddTemporaryChild(FileAndDir_EM.FileNode_df("Log"))
    ...
    def ConstractFileTree_Samples(self):
        self.Dir_Samples = self.Dir_Project.AddChild(FileAndDir_EM.DirNode("Smp"))
        
    def ConstractFileTree_Samples_Branch(self, index: int):
        self.Dir_Samples_Branch = self.Dir_Samples.AddChild(FileAndDir_EM.DirNode("Smp" + str(index)))
        
    def ConstractFileTree_Points_Branch(self, index: int):
        self.Dir_Points_Branch = self.Dir_Samples_Branch.AddChild(FileAndDir_EM.DirNode("Pnt" + str(index)))
        self.CSV_Point_Param = self.Dir_Points_Branch.AddChild(FileAndDir_EM.FileNode_dict("Param"))
        
    def ConstractFileTree_Logs(self):
        self.Dir_Logs = self.Dir_Project.AddChild(FileAndDir_EM.LogDirNode("Log"))
        
    def ConstractFileTree_Logs_Branch(self, f_new: bool = False, tag: str = ""):
        if f_new: self.Dir_Logs_Branch = self.Dir_Logs.AddChild(FileAndDir_EM.DirNode("Log"), FileAndDir_EM.Manager.getDate(), tag)
        else: self.Dir_Logs_Branch = self.Dir_Logs.SearchLastChild("Log")

        self.CSV_Logs_Log = self.Dir_Logs_Branch.AddChild(FileAndDir_EM.FileNode_df("Log"))
        
    def ConstractFileTree_Charts(self):
        self.Dir_Charts = self.Dir_Project.AddChild(FileAndDir_EM.LogDirNode("Charts"))
        
    def ConstractFileTree_Charts_Branch(self, f_new: bool = False, tag: str = ""):
        if f_new: self.Dir_Charts_Branch = self.Dir_Charts.AddChild(FileAndDir_EM.DirNode("Charts"), FileAndDir_EM.Manager.getDate(), tag)
        else: self.Dir_Charts_Branch = self.Dir_Charts.SearchLastChild("Charts")

        self.CSV_Charts_Param = self.Dir_Charts_Branch.AddChild(FileAndDir_EM.FileNode_dict("Param"))
