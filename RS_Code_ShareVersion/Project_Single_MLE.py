...
if __name__ == '__main__':
    ...
    Evaluation_Lyapunov.Evaluation_MLE({
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        "MLE_Epsilon" : 1e-7,                                 #摂動
        "MLE_T_Task" : Task.Task_NDLorenz,                      #評価用タスク（Type型）
        "MLE_T_Model" : Model.Model_NormalESN,                  #モデル（Type型）
        "MLE_T_Output" : Project_Single_MLE.Output_Single_MLE_2023_07_08_17_12,#作図出力（Type型）
        ...
        "Model_HetAESN_LeakingRate" : [0, 0.1, 1.0],  #リーク率配列->サブリザバーの数(必ずリスト型)
        ...
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        "DirPath_Project" : "./Results/Project_Single_MLE_2023_07_08_17_26",
        
        "MLE_F_OutputCharts" : True,                        #図の出力フラグ
        "MLE_F_OutputCharts_MMLEWaves" : True,              #入出力＆誤差波形図の出力フラグ
...
class Output_Single_MLE_2023_07_08_17_12(Output.Output):
    """
    MLEのデバッグ用
    """
    #コンストラクタ
    ...
    def __call__(self, result_param): 
        ...
        if self.F_OutputLog : 
            print("< MLE >")
            for i in range(len(result_param["MLE_R_MLE"])):
                print(result_param["MLE_R_PathShowName"][i] + 
                      "(" + result_param["MLE_R_PathName"][i] + ") : " + 
                      str(result_param["MLE_R_MLE"][i]))
        ...
        if self.F_OutputCharts:
            ...
            if self.F_OutputCharts_MMLEWaves:
                ...
                ax.grid(True)
                for i in range(MMLE.shape[1]):
                    ax.plot(T, MMLE[:, i], label = result_param["MLE_R_PathShowName"][i], lw = LineWidth)
                ax.legend()
    ...
    def ConstractFileTree_Charts_Branch(self, f_new: bool = False, tag: str = ""):
        self.Dir_Results_Branch = self.Dir_Results.AddChild(FileAndDir.DirNode("MLE_"), FileAndDir.Manager.getDate(), tag)

        self.CSV_Charts_Param = self.Dir_Results_Branch.AddChild(FileAndDir.FileNode_dict("ResultAndParam"))
        self.Plt_Charts_MMLEWaves = self.Dir_Results_Branch.AddChild(FileAndDir.FileNode_plt("MMLEWaves"))
