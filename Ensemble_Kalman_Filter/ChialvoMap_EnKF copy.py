...
b_true  = 0.6     # x 依存項の係数 b（推定かつ未知対象：真のパラメータ）
...
Qx  = 0.0026       # x のシステムノイズ分散
Qy  = 0.001       # y のシステムノイズ分散
Qb  = 1e-5        # b のランダムウォークノイズ分散

# 観測行列（x のみ観測）
C = np.array([1.0, 0.0, 0.0])   # y_obs = [1,0,0]·[x, y, b]ᵀ + noise
...
xep[2,:] = 0.0 + 0.05 * np.random.randn(M)   # b の事前平均を 0 に設定
