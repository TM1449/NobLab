...
class Normal:
    ...
    def Load_AllLogs(self, save_type: str) -> list:
        ...
        if Paths is not None:
            for path in Paths:
                Dir_Logs_Branch = self.Dir_Logs.AddTemporaryChild(FileAndDir_No.DirNode(path))
                Dir_Logs_Branch_Log = Dir_Logs_Branch.AddTemporaryChild(FileAndDir_No.FileNode_df("Log"))
    ...
    def ConstractFileTree_Root(self):
        self.Dir_Root = FileAndDir_No.RootNode("./Normal_Reservoir")
        self.Dir_Results = self.Dir_Root.AddChild(FileAndDir_No.DirNode("Results"))
        
        self.ConstractFileTree_Project()
        
    def ConstractFileTree_Project(self):
        self.Dir_Project = self.Dir_Results.AddChild(FileAndDir_No.DirNode("GridSearch" + self.ProjectName + self.ProjectDate))
        self.CSV_Project_Param = self.Dir_Project.AddChild(FileAndDir_No.FileNode_dict("Param"))
    ...
    def ConstractFileTree_Samples(self):
        self.Dir_Samples = self.Dir_Project.AddChild(FileAndDir_No.DirNode("Smp"))
        
    def ConstractFileTree_Samples_Branch(self, index: int):
        self.Dir_Samples_Branch = self.Dir_Samples.AddChild(FileAndDir_No.DirNode("Smp" + str(index)))
        
    def ConstractFileTree_Points_Branch(self, index: int):
        self.Dir_Points_Branch = self.Dir_Samples_Branch.AddChild(FileAndDir_No.DirNode("Pnt" + str(index)))
        self.CSV_Point_Param = self.Dir_Points_Branch.AddChild(FileAndDir_No.FileNode_dict("Param"))
        
    def ConstractFileTree_Logs(self):
        self.Dir_Logs = self.Dir_Project.AddChild(FileAndDir_No.LogDirNode("Log"))
        
    def ConstractFileTree_Logs_Branch(self, f_new: bool = False, tag: str = ""):
        if f_new: self.Dir_Logs_Branch = self.Dir_Logs.AddChild(FileAndDir_No.DirNode("Log"), FileAndDir_No.Manager.getDate(), tag)
        else: self.Dir_Logs_Branch = self.Dir_Logs.SearchLastChild("Log")

        self.CSV_Logs_Log = self.Dir_Logs_Branch.AddChild(FileAndDir_No.FileNode_df("Log"))
        
    def ConstractFileTree_Charts(self):
        self.Dir_Charts = self.Dir_Project.AddChild(FileAndDir_No.LogDirNode("Charts"))
        
    def ConstractFileTree_Charts_Branch(self, f_new: bool = False, tag: str = ""):
        if f_new: self.Dir_Charts_Branch = self.Dir_Charts.AddChild(FileAndDir_No.DirNode("Charts"), FileAndDir_No.Manager.getDate(), tag)
        else: self.Dir_Charts_Branch = self.Dir_Charts.SearchLastChild("Charts")

        self.CSV_Charts_Param = self.Dir_Charts_Branch.AddChild(FileAndDir_No.FileNode_dict("Param"))
